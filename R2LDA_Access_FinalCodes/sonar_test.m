...
%training data
C=[0,1];     %class labels,  0=Rocks (R), 1=Mines (M) 
p=60;
load('sonar.mat');
data=alldata(:,1:p);
labels=alldata(:,end);
...
perm=randperm(n0_full);
XX0=XX0(perm,:); 
perm=randperm(n1_full);
XX1=XX1(perm,:); 
...
nts=50;
ntr=10:10:80;         %no of training samples  10:10:70
max_iter=500;         %no of trials 
for stdn = [0 0.1 0.2 0.3];   
randn('state', 10);                
rand('state', 10);   
...
%         rziVec=zeros(20, 1);
%         for kk=1:10
%         rziVec(kk)=covcopra(U,dd,X1(kk,:)' - 1/2*m_plus,len1);
%         rziVec(kk+10)=covcopra(U,dd,X1(kk+ntr0,:)' - 1/2*m_plus,len1);
%         end
%         rzi0=median(rziVec);
...
% figpth = strcat(pth, filesep, "Sonar_", stdnstr, '.fig');
% epspth = strcat(pth, filesep, "Sonar_", stdnstr, '.epsc')
% savefig(figpth);
% saveas(gcf, epspth)
