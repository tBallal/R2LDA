...
%%  generate training/testing data from Phoneme dataset
%training data
for phNo=1:3 %  %class labels/digits [1,7],[5,8],[7,9]
if phNo==1    
    C=[1,4];    
elseif phNo==2
    C=[1,5];
    ...
    C=[2,3];
end    %class labels, {1,2,3,4,5}, five phonemes [1,2]; [1,3], [4,5]
p=256;                           
load('phoneme.mat');
labels=alldata(:,end);
data=alldata(:,1:p);
...
n0_full=length(indx0);          %no of samples in class 0
n1_full=length(indx1);          %no of samples in class 1
ntr0_full=round(n0_full/2);
ntr1_full=round(n1_full/2);
nts0_full=n0_full-ntr0_full;
nts1_full=n1_full-ntr1_full;
nrm = max(abs(data(:)));
XX0=data(indx0,:)/nrm;              %data matrix of class 0
XX1=data(indx1,:)/nrm;              %data matrix of class 1
XXtr0=XX0(1:ntr0_full,:);
XXtr1=XX1(1:ntr1_full,:);
XXts0=XX0(ntr0_full+1:n0_full,:);
XXts1=XX1(ntr1_full+1:n1_full,:);
clear alldata data labels indx0 indx1
...
nts= 500;
ntr= 40:40:280;       %no of training samples 
max_iter=500;        %no of trials 
for stdn = [0 0.1 0.2 0.3];  %[0 1 2 4 8]/20
randn('state', 10);                
rand('state', 10);
...
for ii=1:length(ntr)
    N1=ntr(ii);
    disp(['Phonemes: ' num2str([stdn N1])])
%     ntr0=round( N1*(n0_full/n_full) );            %training samples for class 0
%     ntr1=N1-ntr0; 
...
figpth = strcat(pth, filesep, "Phoneme", num2str(Co(1)), num2str(Co(2)),'_', stdnstr, '.fig');
epspth = strcat(pth, filesep, "Phoneme", num2str(Co(1)), num2str(Co(2)),'_', stdnstr, '.epsc');
savefig(figpth);
saveas(gcf, epspth);
