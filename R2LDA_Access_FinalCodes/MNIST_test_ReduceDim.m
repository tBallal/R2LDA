...
%% apply feature selection/reduction using filter approach
[hh,pp,cii,statt]=ttest2(XXtr0,XXtr1,'Vartype','unequal');
% figure(1), subplot(224), ecdf(pp), 
% xlabel('p value'), ylabel('CDF'), title('[1,7]'), grid on,
% no_sig_fs=length(find(pp<0.05)),   %features with p>0.05 are insignificant
nfs=50;     %no of features to be used
[~,fsidx]=sort(pp);     %features indices
XXtr0=XXtr0(:,fsidx(1:nfs)); XXtr1=XXtr1(:,fsidx(1:nfs));   %red dim train data
XXts0=XXts0(:,fsidx(1:nfs)); XXts1=XXts1(:,fsidx(1:nfs));   %red dim test data


%% simulation parameters
p=nfs;              %dimensions/features
ntr=40:40:360;      %no of training samples 50:50:400; 
...
% figpth = strcat(pth, filesep, "MNIST79DR_", stdnstr, '.fig');
% epspth = strcat(pth, filesep, "MNIST79DR_", stdnstr, '.epsc')
% savefig(figpth);
% saveas(gcf, epspth)
% end